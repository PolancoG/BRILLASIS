...
try {
    ...
    if ($metodo_pago === 'ahorro') {
        $stmt = $conn->prepare("SELECT saldo FROM ahorro WHERE cliente_id = :cliente_id");
        ...
        if ($ahorro['saldo'] < $cuota['monto_cuota']) {
        ...
        $nuevo_saldo = $ahorro['saldo'] - $cuota['monto_cuota'];
        $stmt = $conn->prepare("UPDATE ahorro SET saldo = :nuevo_saldo WHERE cliente_id = :cliente_id");
